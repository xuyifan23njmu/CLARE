...
# 读取Excel文件
xls = pd.ExcelFile('prompt_origion_v2.0.xlsx')
df_prompt = pd.read_excel(xls, 'prompt')
...
for condition_key, condition_value in CONDITION.items():
    for location_key, location_value in LOCATION.items():
        df_english = pd.DataFrame(columns=['English', 'English_a', 'model_name', 'disease', 'location'])
        for prefix in english_column:
            ...
            'English': prompts,
            'English_a': answers,
            'model_name': ['DeepSeek-R1-Distill-Qwen-7B'] * len(prompts),
        ...
        # 保存生成的数据到Excel文件
        df_english.to_excel(f'output\DeepSeek-R1-Distill-Qwen-7B-{condition_value}-{location_value}-english.xlsx', index=False)
...
for condition_key, condition_value in CONDITION.items():
    for location_key, location_value in LOCATION.items():
        df_chinese = pd.DataFrame(columns=['Chinese', 'Chinese_a', 'model_name', 'disease', 'location'])
        for prefix in chinese_column:
        ...
        # 保存生成的数据到Excel文件
        df_chinese.to_excel(f'output\DeepSeek-R1-Distill-Qwen-7B-{condition_value}-{location_value}-chinese.xlsx', index=False)
...
# 保存生成的数据到Excel文件
df.to_excel('output\DeepSeek-R1-Distill-Qwen-7B.xlsx', index=False)
'''
# 记录程序结束运行的时间
